...
Start with just your MCP host. Codex: codex mcp add murmur --url https://api.usemurmur.dev/setup/mcp . Claude Code: claude mcp add --transport http --scope user murmur https://api.usemurmur.dev/setup/mcp . OpenCode, Cursor, and Pi can add the same Streamable HTTP setup URL in their native MCP configuration. Pi requires the third-party pi-mcp-adapter listed in Pi's official package catalog; Murmur never installs it. Restart and call get_setup_guide. This public connection is read-only.
