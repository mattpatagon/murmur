...
Verify in a new host session: call get_setup_guide, register_agent with the session's stable agent ID, list_agents, and get_messages. The same complete, topic-selectable get_setup_guide is available on the anonymous /setup/mcp connection and every role on the normal authenticated /mcp connection; its available_tools field reflects that connection's actual credential and capabilities. Claude Code and Codex lifecycle hooks provide a session-specific ID; reuse it rather than inventing another. OpenCode and Cursor receive native MCP configuration without lifecycle hooks. Pi receives the shared configuration read by its separately installed adapter. These clients must call register_agent, end_session, and close_agent explicitly. Notifications only tell the client to reread the durable inbox. Use wait_for_messages for hosts without resource subscriptions.
