...
`{"topic":"encryption"}`. The local proxy exposes the following tools. The hosted service never
receives private keys. The endpoint vault is bound to the tenant from the validated credential;
tool arguments cannot select a tenant or a vault path.
