...
// Windows scenarios launch up to 36 bounded ACL processes across signup and retry cases.
...
test(
  "signup configuration keeps credentials on the validated custom service in each shell",
  async (): Promise<void> => {
...
test(
  "signup keeps owner and worker credentials private, separate, and durable across reruns",
  async (): Promise<void> => {
...
test(
  "a lost registration response retries the exact saved high-entropy registration",
  async (): Promise<void> => {
...
test(
  "worker failure preserves the owner credential and resumes without registering again",
  async (): Promise<void> => {
...
test(
  "signup rejects server responses with the wrong tenant, organization, or credential role",
  async (): Promise<void> => {
...
test(
  "signup validates options, private paths, and retained checkpoint ownership before network work",
  async (): Promise<void> => {
    ...
    try {
      ...
      expect(
        (): SignupOptions =>
          parseSignupArguments(["--slug", "example-org", "--slug", "another-org"]),
      ).toThrow("Usage");
